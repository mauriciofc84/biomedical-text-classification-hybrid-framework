...
plt.rcParams['pdf.fonttype'] = 42  # Exporta los textos como fuentes reales, no como caminos/curvas mapeadas
plt.rcParams['ps.fonttype'] = 42   # Mismo criterio para archivos PostScript si fuesen necesarios
...
LITERAL_STOPWORDS = {
    ...
    'del', 'se', 'las', 'por', 'un', 'para', 'con', 'no', 'una', 'su', 'al', 'lo', 'como'
}
...
if SHAP_AVAILABLE:
    ...
    try:
        ...
        if not df_plot.empty:
            ...
            plt.xlabel("Cumulative Impact", fontsize=18, labelpad=10)
            plt.yticks(fontsize=16)
            plt.legend(fontsize=12)
            plt.title(f"Global Importance (SHAP) - {MODEL_NAME}", fontsize=22, pad=15)
            plt.tight_layout()
            plt.savefig(os.path.join(DIR_IMG, f"grafico_qlik_shap_top5_{MODEL_NAME}.pdf"), format='pdf', bbox_inches='tight', dpi=300)
            plt.close()
